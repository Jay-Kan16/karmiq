export type Role="customer"|"worker"|"admin"; export type BookingStatus="SEARCHING"|"REQUESTED"|"SCHEDULED"|"MATCHING"|"WORKER_ASSIGNED"|"ACCEPTED"|"ON_THE_WAY"|"ARRIVED"|"SERVICE_STARTED"|"COMPLETED"|"CANCELLED"|"PAYMENT"|"RATING"|"REJECTED"; export type PaymentStatus="PENDING"|"PAID"|"FAILED"; export type Availability="online"|"offline"|"busy";
...
export interface Booking{id:string;customerId:string;workerId?:string;serviceId:string;serviceName:string;location:{lat:number;lng:number;address:string};date?:string;time?:string;scheduledDate?:string;scheduledTime?:string;emergency:boolean;description?:string;fareMin:number;fareMax:number;fare:number;baseFare?:number;extraCharges?:number;extraChargesReason?:string;status:BookingStatus;rejectedReason?:string;paymentStatus:PaymentStatus;createdAt:string;eta?:number;distance?:number;matchScore?:number}
